...
public class HistoryController {
	 ...
	 * 
	 * <p>
	 * 指定された tab の値をもとに Service を呼び分け、
	 * 取得した履歴一覧を画面表示用として Model に設定する。
	 * </p>
	 *
	...
	public String history(@RequestParam(name = "tab", defaultValue = "all") String tab, Model model) {		
		...
		List<ResultHistoryDto> resultHistoryDto;
		//tab によって Service を呼び分ける
		switch (tab) {
		case "janken":
			resultHistoryDto = historyService.findByGameType(userId,GameType.JANKEN);
			break;
		case "acchi":
			resultHistoryDto = historyService.findByGameType(userId,GameType.ACCHI);
			break;
		default:
			resultHistoryDto = historyService.findAll(userId);
		}
		model.addAttribute("rows", resultHistoryDto);
