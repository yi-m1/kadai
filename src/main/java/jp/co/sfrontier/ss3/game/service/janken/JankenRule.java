 ...
 */
@Service
public class JankenRule implements GameRule<Hand, BattleResult> {

	@Override
	public BattleResult judge(Hand player1Action, Hand player2Action) {

		// 引き分け
		if (player1Action == player2Action) {
			return new BattleResult(ResultCode.DRAW);
		}

		// player1 勝ち
		if (player1Action == Hand.ROCK
				&& player2Action == Hand.SCISSORS) {
			return new BattleResult(ResultCode.WIN);
		}
		...
		// それ以外は負け
		return new BattleResult(ResultCode.LOSE);
	}
